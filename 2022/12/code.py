...
solution_1 = networkx.dijkstra_path_length(g, start[0]*len(input[0]) + start[1], end[0]*len(input[0]) + end[1])
...
for i in range(len(input)):
    for j in range(len(input[i])):
        ...
        if input[i][j] == 'a':
            try:
                temp = networkx.dijkstra_path_length(g, i*len(input[0]) + j, end[0]*len(input[0]) + end[1])
