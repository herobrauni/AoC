...
with open(os.getcwd() + "/2022/24/input.txt", 'r') as f:
    input = f.read()
    input = input.split("\n")
    # input = []
...
result = np.where(grids[0] == 0)
free_fields[0] = list(zip(result[0], result[1]))


for free_field in free_fields[0]:
    G.add_node((0,free_field[0],free_field[1]))

i = 1
while True:
    # print(i)
    grids[i] = move_storms(grid, i)
    result = np.where(grids[i] == 0)
    free_fields[i] = list(zip(result[0], result[1]))
    for free_field in free_fields[i]:
        G.add_node((i,free_field[0],free_field[1]))
        for d in [(1,0),(-1,0),(0,-1),(0,1),(0,0)]:
            prev =  a(free_field,d)
            if prev[0] not in range(len(grids[0])) or prev[1] not in range(len(grids[0][0])):
                continue
            if prev in free_fields[i-1]:
                G.add_edge((i-1,prev[0],prev[1]), (i,free_field[0],free_field[1]))
