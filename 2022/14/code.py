...
with open(os.getcwd() + "/2022/14/input.txt", 'r') as f:
    input = f.read()
    input = input.split("\n")
    # input = []
...
# PART 1
grid = np.zeros(((max_x-min_x+1)*(max_y+1),), dtype=int).reshape(max_y+1, (max_x-min_x+1))
...
scale = (max_y+2+1)*2
# 289
grid = np.zeros(((max_x-min_x+1+scale)*(max_y+2+1),), dtype=int).reshape(max_y+2+1, (max_x-min_x+1+scale))
