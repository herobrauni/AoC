...
with open(os.getcwd() + "/2022/22/input.txt", 'r') as f:
    input = f.read()
    input = input.split("\n\n")
    # input = []
...
for instr in instructions:
	...
	if re.search('\d', instr) != None:
		...
		for i in range(instr):
			...
			if a(pos,directions[d])[0] not in range(len(grid)) or a(pos,directions[d])[1] not in range(w_max):
				pos = find_wrap(grid, pos, directions[d])
			elif grid[a(pos,directions[d])] == ' ':
				pos = find_wrap(grid, pos, directions[d])
			elif grid[a(pos,directions[d])] == '.': #or grid[a(pos,directions[d])] in direction_markers:
...
side_0 = [np.array([1,2,3,4]),np.array([5,6,7,8]),np.array([9,10,11,12]),np.array([13,14,15,16])]
	...
	if re.search('\d', instr) != None:
		...
		for i in range(instr):
			...
			if cs[side][a(pos,directions[d])] not in [0,8]:
				# print("wrapping")
				pos, side, cs = move_cube2(cs, pos, directions[d], side) # og_pos, og_side, og_cs
			elif cs[side][a(pos,directions[d])] == 0: #or grid[a(pos,directions[d])] in direction_markers:
				pos = a(pos,directions[d])
...
tmp = len(cs[side])
match side:
	case 1: offset = (1,tmp+1) # 1,51
	case 2: offset = (1,tmp*2+1) # 1,101
	case 3: offset = (tmp+1,tmp+1) # 51,51
	case 4: offset = (tmp*2+1,tmp+1) # 101,51
	case 5: offset = (tmp*2+1,1) # 101,1
	case 6: offset = (tmp*3+1,1) # 151,1
