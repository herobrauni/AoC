...
def en(a, empN):
    ...
    while abc < len(empN):
        ...
        for x in [left, right, down, up, back, forward]:
            if (
                list(np.add(a, x)) not in ic
                and tuple(np.add(a, x)) not in empN
                and all(
                    [
                        list(np.add(a, x))[0] in xm,
                        list(np.add(a, x))[1] in ym,
                        list(np.add(a, x))[2] in zm,
                    ]
                )
            ):
...
for i in range(len(blub)):
    x = 6
    if [blub[i][0] + 1, blub[i][1], blub[i][2]] in blub + ic:
        x -= 1
    if [blub[i][0], blub[i][1] + 1, blub[i][2]] in blub + ic:
        x -= 1
    if [blub[i][0], blub[i][1], blub[i][2] + 1] in blub + ic:
        x -= 1
    if [blub[i][0] - 1, blub[i][1], blub[i][2]] in blub + ic:
        x -= 1
    if [blub[i][0], blub[i][1] - 1, blub[i][2]] in blub + ic:
