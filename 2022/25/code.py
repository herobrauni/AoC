...
with open(os.getcwd() + "/2022/25/input.txt", 'r') as f:
    input = f.read()
    input = input.split("\n")
    # input = []
...
def trans_numbers_rev(solution_1):
	...
	i = 0
	while True:
		if solution_1 in range(sum([x[4] for x in abc[0:i]]),sum([x[0] for x in abc[0:i]])+1):
			break
		i += 1
	...
	for i in range(i,0,-1):
		...
		for j in range(len(abc[i])):
			...
			if s - abc[i][j] in range(sum([x[4] for x in abc[0:i]]),sum([x[0] for x in abc[0:i]])+1):
				s = s - abc[i][j]
